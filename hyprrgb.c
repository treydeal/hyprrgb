...
static void on_output_geometry(void *data, struct wl_output *o, int32_t x, int32_t y,
                               int32_t pw, int32_t ph, int32_t subpixel,
                               const char *make, const char *model, int32_t transform) {
    (void)data; (void)o; (void)x; (void)y; (void)pw; (void)ph;
    (void)subpixel; (void)make; (void)model; (void)transform;
}
static void on_output_mode(void *data, struct wl_output *o, uint32_t flags,
                           int32_t w, int32_t h, int32_t refresh) {
    (void)data; (void)o; (void)flags; (void)w; (void)h; (void)refresh;
}
static void on_output_done(void *data, struct wl_output *o) { (void)data; (void)o; }
...
static void on_ctm_blocked(void *data, struct hyprland_ctm_control_manager_v1 *mgr) {
    (void)mgr;
    struct state *s = data;
    s->blocked = true;
...
static void on_global(void *data, struct wl_registry *registry, uint32_t name,
    ...
    if (strcmp(interface, wl_output_interface.name) == 0) {
        ...
        s->ctm = wl_registry_bind(registry, name, &hyprland_ctm_control_manager_v1_interface, bind_version);
        hyprland_ctm_control_manager_v1_add_listener(s->ctm, &ctm_listener, s);
    }
}
...
static void apply_rgb(const char *r_s, const char *g_s, const char *b_s) {
    if (!st.ctm) {
...
int main(int argc, char **argv) {
    ...
    if (!st.ctm) {
        fprintf(stderr, "hyprland_ctm_control_manager_v1 not advertised\n");
        return 1;
    }
    ...
    while (st.running) {
        ...
        if (!strcmp(line, "reset")) {
            apply_rgb("100", "100", "100");
            continue;
        }

        char r[64], g[64], b[64];
        if (sscanf(line, "%63s %63s %63s", r, g, b) == 3)
            apply_rgb(r, g, b);
        else
            fprintf(stderr, "Enter: <red%%> <green%%> <blue%%>\n");
    }

    if (st.ctm)
        hyprland_ctm_control_manager_v1_destroy(st.ctm);
    for (size_t i = 0; i < st.outputs_len; ++i)
        if (st.outputs[i].output)
            wl_output_destroy(st.outputs[i].output);
    if (st.registry)
